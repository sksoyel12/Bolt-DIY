...
/**
 * Connect the repository tools with the OAuth access token returned by
 * Firebase's GitHub popup. The token is never typed or pasted by the user.
 */
export async function connectGitHubWithOAuthToken(token: string): Promise<GitHubConnection> {
  ...
  if (!response.ok) {
    throw new Error(`GitHub OAuth connection failed: ${response.status}`);
  }

  const userData = (await response.json()) as GitHubConnection['user'];
