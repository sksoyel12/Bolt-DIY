...
async function llmCallAction({ context, request }: ActionFunctionArgs) {
  ...
  const providerSettings: Record<string, IProviderSetting> = getProviderSettingsFromCookie(
  ...
  if (streamOutput) {
    ...
    try {
      ...
      const providerInfo = PROVIDER_LIST.find((candidate) => candidate.name === provider.name);

      if (!providerInfo) {
        throw new Error('Provider not found');
      }

      let resolvedProvider = providerInfo;
      let resolvedModelDetails = modelDetails;

      if (provider.name === 'Together' && !hasProviderKey('Together')) {
        const fallbackProviderName = ['Groq', 'Google', 'OpenAI'].find(hasProviderKey);
        const fallbackProvider = fallbackProviderName
          ? PROVIDER_LIST.find((candidate) => candidate.name === fallbackProviderName)
          : undefined;

        if (fallbackProvider) {
          const fallbackModels = models.filter((candidate) => candidate.provider === fallbackProvider.name);

          if (fallbackModels.length > 0) {
            resolvedProvider = fallbackProvider;
            resolvedModelDetails = fallbackModels[0];
          }
        }
      }

      const dynamicMaxTokens = getCompletionTokenLimit(resolvedModelDetails);

      // Validate token limits before making API request
      const validation = validateTokenLimits(resolvedModelDetails, dynamicMaxTokens);
      ...
      logger.info(`Generating response Provider: ${resolvedProvider.name}, Model: ${resolvedModelDetails.name}`);

      // DEBUG: Log reasoning model detection
      const isReasoning = isReasoningModel(resolvedModelDetails.name);
      logger.info(`DEBUG: Model "${resolvedModelDetails.name}" detected as reasoning model: ${isReasoning}`);
      ...
      const baseParams = {
        ...
        model: resolvedProvider.getModelInstance({
          model: resolvedModelDetails.name,
      ...
      logger.info(
        `DEBUG: Final params for model "${resolvedModelDetails.name}":`,
        JSON.stringify(
          {
            isReasoning,
