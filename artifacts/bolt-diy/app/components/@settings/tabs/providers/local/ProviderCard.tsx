              ...
              {provider.settings.enabled && (
                <div className="space-y-2">
                  ...
                  {provider.name === 'OpenAILike' && (
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-bolt-elements-textPrimary">Custom model names</label>
                      <input
                        type="text"
                        defaultValue={provider.settings.OPENAI_LIKE_API_MODELS || ''}
                        placeholder="llama3.2, qwen2.5-coder"
                        className="w-full px-4 py-3 rounded-lg text-sm bg-bolt-elements-background-depth-4 border border-bolt-elements-borderColor text-bolt-elements-textPrimary placeholder-bolt-elements-textTertiary focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500 transition-all duration-200"
                        onBlur={(event) => onUpdateModels(event.currentTarget.value)}
                        onKeyDown={(event) => {
                          if (event.key === 'Enter') {
                            onUpdateModels(event.currentTarget.value);
                            event.currentTarget.blur();
                          }
                        }}
                      />
                      <p className="text-xs text-bolt-elements-textSecondary">
                        Optional. Use this for a local OpenAI-compatible server that does not expose /models. No API
                        key is required for unauthenticated endpoints.
                      </p>
                    </div>
                  )}
                </div>
